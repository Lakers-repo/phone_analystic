 ...
 * @since 1.0.0
 */
public class ActiveBrowserUserRunner implements Tool {
    private static final Logger logger = Logger.getLogger(ActiveBrowserUserRunner.class);
    private Configuration conf = new Configuration();

    //主函数---入口
    public static void main(String[] args){
        try {
            ToolRunner.run(new Configuration(),new ActiveBrowserUserRunner(),args);
        } catch (Exception e) {
            logger.warn("Active_Browser_NEW_USER TO MYSQL is failed !!!",e);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"Active_Browser_NEW_USER TO MYSQL");

        job.setJarByClass(ActiveBrowserUserRunner.class);

        //设置map相关参数
        job.setMapperClass(ActiveBrowserUserMapper.class);
