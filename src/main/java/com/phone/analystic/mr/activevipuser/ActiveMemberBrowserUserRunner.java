 ...
 * @since 1.0.0
 */
public class ActiveMemberBrowserUserRunner implements Tool {
    private static final Logger logger = Logger.getLogger(ActiveMemberBrowserUserRunner.class);
    private Configuration conf = new Configuration();

    //主函数---入口
    public static void main(String[] args){
        try {
            ToolRunner.run(new Configuration(),new ActiveMemberBrowserUserRunner(),args);
        } catch (Exception e) {
            logger.warn("Active_Browser_MEMBER_USER TO MYSQL is failed !!!",e);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"Active_Browser_MEMBER_USER TO MYSQL");

        job.setJarByClass(ActiveMemberBrowserUserRunner.class);

        //设置map相关参数
        job.setMapperClass(ActiveMemberBrowserUserMapper.class);
