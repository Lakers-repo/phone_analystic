...
import com.phone.analystic.mr.activeuser.ActiveUserMapper;
import com.phone.analystic.mr.activeuser.ActiveUserReducer;
 ...
 * @since 1.0.0
 */
public class ActiveMemberUserRunner implements Tool {
    private static final Logger logger = Logger.getLogger(ActiveMemberUserRunner.class);
    private Configuration conf = new Configuration();

    //主函数---入口
    public static void main(String[] args){
        try {
            ToolRunner.run(new Configuration(),new ActiveMemberUserRunner(),args);
        } catch (Exception e) {
            logger.warn("Active_MEMBER_USER TO MYSQL is failed !!!",e);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"Active_MEMBER_USER TO MYSQL");

        job.setJarByClass(ActiveMemberUserRunner.class);

        //设置map相关参数
        job.setMapperClass(ActiveMemberUserMapper.class);
        job.setMapOutputKeyClass(StatsUserDimension.class);
        job.setMapOutputValueClass(TimeOutPutValue.class);

        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(ActiveMemberUserReducer.class);
