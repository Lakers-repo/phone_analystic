...
/**
 * 〈一句话功能简述〉<br> 
 * 〈ActiveMemberBrowserUserMapper---mapper函数 简单的封装〉
 *
 * @author 14751
 * @create 2018/9/19 
 * @since 1.0.0
 * 浏览器模块下的活跃会员---当天PV事件中去重的UMID个数
 */
public class ActiveMemberBrowserUserMapper extends Mapper<LongWritable,Text,StatsUserDimension,TimeOutPutValue> {
    private static final Logger logger = Logger.getLogger(ActiveMemberBrowserUserMapper.class);
    private StatsUserDimension k = new StatsUserDimension();
    private TimeOutPutValue v = new TimeOutPutValue();

    private KPIDimension activeMemberBrowserUserKpi = new KPIDimension(KpiType.BROWSER_ACTIVE_MEMBER.kpiName);
    @Override
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(StringUtils.isNotEmpty(en) && en.equals(EventLogContant.EventEnum.PAGEVIEW.alias)){
            ...
            if(StringUtils.isEmpty(serverTime) || StringUtils.isEmpty(umid)){
                logger.info("serverTime & active_browser_mem_umid is null serverTime:"+serverTime+".active_browser_mem_umid"+umid);
                return;
            }
            ...
            statsCommonDimension.setKpiDimension(activeMemberBrowserUserKpi);
