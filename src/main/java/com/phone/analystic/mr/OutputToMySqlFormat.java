...
//public class DBOutputFormat<K extends DBWritable, V> extends OutputFormat<K, V>---参考DBOutputFormat
    //这里的key必须继承自一个类---这里也就是父类（key---StatsBaseDimension）
public class OutputToMySqlFormat extends OutputFormat<StatsBaseDimension,OutPutWritable> {
    ...
    public static class OutputToMysqlRecordWritter extends RecordWriter<StatsBaseDimension,OutPutWritable>{
        ...
        /**
         * 写---这里是为了输出到MySQL中的接口做准备（IOutputWritter）
         * 关键方法
         * @param key
         * @param value
         * @throws IOException
        ...
        public void write(StatsBaseDimension key, OutPutWritable value) throws IOException, InterruptedException {
            ...
            try {
                ...
                //com.phone.analystic.mr.newuser.NewUserOutputWritter
                Class<?> classz = Class.forName(className);//将字符串转换成类---反射
                //返回的是Object类型，需要强转，一种多态的体现，代码的健壮性，也可以用他的实现类取强转
                IOutputWritter writter = (IOutputWritter) classz.newInstance();  //用来调用对象里面的方法
...
//                    this.conn.commit(); //提交批处理执行(注意点.这里不需要加)
