 ...
 * @since 1.0.0
 */
public class VipBrowserUserRunner implements Tool {
    private static final Logger logger = Logger.getLogger(VipBrowserUserRunner.class);
    private Configuration conf = new Configuration();

    //主函数---入口
    public static void main(String[] args){
        try {
            ToolRunner.run(new Configuration(),new VipBrowserUserRunner(),args);
        } catch (Exception e) {
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"Browser_NEW_MEMBER TO MYSQL");

        job.setJarByClass(VipBrowserUserRunner.class);

        //设置map相关参数
        job.setMapperClass(VipBrowserUserMapper.class);
        job.setMapOutputKeyClass(StatsUserDimension.class);
        job.setMapOutputValueClass(TimeOutPutValue.class);

        //设置reduce相关参数
        //设置reduce端的输出格式类
        job.setReducerClass(VipUserReducer.class);
...
//        return job.waitForCompletion(true)? 0:1;
        if(job.waitForCompletion(true)){
            computeNewTotalMember(job);//计算总的新增会员---浏览器模块
            return 0;
        }else{
            return 1;
        }
    ...
    private void computeNewTotalMember(Job job) {
        ...
        try {
            ...
            if(yesterdayDimensionId > 0){
                ps = conn.prepareStatement(conf.get(GlobalConstants.PREFIX_TOTAL+"new_browser_total_member"));
                //给ps赋值
                ...
                while(rs.next()){
                    ...
                    //存储
                    map.put(platformId+"-"+browserId,totalNewMemberUser);
                }
            }

            if(nowDimensionId > 0){
                ps = conn.prepareStatement(conf.get(GlobalConstants.PREFIX_TOTAL+"BROWSER_new_member"));
                //给ps赋值
                ...
                while(rs.next()){
                    ...
                    if(map.containsKey(platformId+"-"+browserId)){
                        newMemberUser += map.get(platformId+"-"+browserId);
                    }
                    //如果没有当天该平台下的总新增用户就是当天新增用户，添加到map中，为后一天计算准备
                    map.put(platformId+"-"+browserId,newMemberUser);
                }
            }

            //更新新增的总用户
            ps = conn.prepareStatement(conf.get(GlobalConstants.PREFIX_TOTAL+"Browser_new_update_member"));
            //给ps赋值
