 ...
 * @since 1.0.0
 */
public class VipUserRunner implements Tool {
    private static final Logger logger = Logger.getLogger(VipUserRunner.class);
    private Configuration conf = new Configuration();

    //主函数---入口
    public static void main(String[] args){
        try {
            ToolRunner.run(new Configuration(),new VipUserRunner(),args);
        } catch (Exception e) {
            logger.warn("Vip_USER TO MYSQL is failed !!!",e);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"Vip_MEMBER TO MYSQL");

        job.setJarByClass(VipUserRunner.class);

        //设置map相关参数
        job.setMapperClass(VipUserMapper.class);
